...
def D_SHADE(func, problem_args, algorithm_args):
    '''
    Input:
        ...
        algorithm_args: a dict. It should contain the following key value pairs:
            ...
            "r_freq": a int indicating the total number of time for population number reduction
    Output:
    ...
    # initialization
    NFE = 0
    current_population_evaluation_num = 0
    history_populations = []
    current_population = []
    ...
    for idx in range(algorithm_args["N_init"]):
        ...
        NFE += 1
        current_population_evaluation_num += 1
    ...
    while NFE < problem_args["MAX_NFE"]:
        ...
        print(f"best value: {min([a.f_x for a in current_population])}")
        ...
        for result in results:
            ...
            NFE += new_NFE
            current_population_evaluation_num += new_NFE
        ...
        # DPSR
        if current_population_evaluation_num > problem_args["MAX_NFE"] / algorithm_args["r_freq"]:
            print("doing DPSR")
            current_population_evaluation_num = 0
            removing_num = math.floor(len(current_population) / 2)
            new_population = []
            for idx in range(removing_num):
                new_population.append(
                    current_population[idx]
                    if current_population[idx].f_x < current_population[len(current_population) - 1 - idx].f_x
                    else current_population[len(current_population) - 1 - idx]
                )
            if len(current_population) % 2 == 1:
                new_population.append(current_population[removing_num])
            current_population = new_population

    return min(current_population, key = lambda x: x.f_x), history_populations

def get_D_SHADE_args(dimension):
    algorithm_args = {
