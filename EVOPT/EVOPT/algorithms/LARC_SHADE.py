...
from sklearn.cluster import MeanShift, HDBSCAN
...
def LARC_SHADE(func, problem_args, algorithm_args):
    '''
    Input:
        ...
        algorithm_args: a dict. It should contain the following key value pairs:
            ...
            "gamma": a float used for probablistic removement
    Output:
    ...
    A = set()
    k = 0
    best_solution = min(current_population, key=lambda s: s.f_x)

    # main loop
    while NFE < problem_args["MAX_NFE"]:
        print(f"{NFE} / {problem_args['MAX_NFE']}")
        print(f"best value: {best_solution.f_x}")
        ...
        # update history best solution
        best_solution = min(current_population + [best_solution], key=lambda s: s.f_x)
        ...
        # KLPSR
        new_population_num = round(
            (algorithm_args["N_min"] - algorithm_args["N_init"]) / problem_args["MAX_NFE"] * NFE
            +
            algorithm_args["N_init"]
        )
        if new_population_num < len(current_population):
            sorted_current_population = sorted(
                current_population,
                key = lambda x: x.f_x,
            )
            # ms = MeanShift(n_jobs = -1)
            ms = HDBSCAN(min_cluster_size = 2, n_jobs = -1)
            ms.fit(
                [p.x for p in sorted_current_population]
            )
            labels = ms.labels_
            max_labels = max(labels) + 1
            for idx in range(len(labels)):
                if labels[idx] == -1:
                    labels[idx] = max_labels
                    max_labels += 1
            clusters = {}
            cluster_rank_sum = {}
            for idx, sol in enumerate(sorted_current_population):
                cluster = labels[idx]
                clusters[cluster] = clusters.get(cluster, []) + [sol]
                cluster_rank_sum[cluster] = cluster_rank_sum.get(cluster, 0) + (idx + 1)
            current_gamma = (algorithm_args["gamma_final"] - 0) / problem_args["MAX_NFE"] * NFE
            for _ in range(len(current_population) - new_population_num):
                max_rank_sum = max([cluster_rank_sum[idx] for idx in clusters.keys()])
                cluster_idx = random.choices(
                    list(clusters.keys()),
                    weights=list(math.exp(cluster_rank_sum[idx] / max_rank_sum * current_gamma) for idx in clusters.keys()),
                    k = 1
                )[0]
                removing_solution = clusters[cluster_idx][-1]
                clusters[cluster_idx] = clusters[cluster_idx][:-1]
                cluster_rank_sum[cluster_idx] -= sorted_current_population.index(removing_solution) + 1
                if len(clusters[cluster_idx]) == 0:
                    clusters.pop(cluster_idx)
                    cluster_rank_sum.pop(cluster_idx)
            current_population = sum(clusters.values(), [])

    return best_solution, history_populations

def get_LARC_SHADE_args(dimension):
    algorithm_args = {
