...
def SHADE(func, problem_args, algorithm_args):
    '''
    Input:
        ...
        algorithm_args: a dict. It should contain the following key value pairs:
            "N": an int indicating the number of the population
            "H": an int indicating the size of M_CR and M_F
            "p_min": a float indicating the minimum value of p, which effects the choice of pbest. It should be less than 0.2
    Output:
    ...
    for idx in range(algorithm_args["N"]):
    ...
    while NFE < problem_args["MAX_NFE"]:
        ...
        # clean A
        while len(A) > len(current_population):
...
def get_SHADE_args(dimension):
    # this setting is for problems of 30 dimensions
    algorithm_args = {
        "N": 100,
        "H": 100,
        "p_min": 0.1,
